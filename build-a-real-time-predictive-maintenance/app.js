...
function saveCsv() {
  if (!state.samples.length) {
    setStatus("No samples to save yet", "warning");
    return;
  }
  ...
  const rows = state.samples.map((sample) => [
    ...
    sample.vibration.toFixed(3),
    Math.round(sample.sound),
    sample.current.toFixed(3),
    sample.motorTemperature.toFixed(2),
    sample.ambientTemperature.toFixed(2),
    Math.round(sample.humidity),
    sample.prediction,
    Math.round(sample.risk),
    sample.diagnosis.title,
    sample.diagnosis.confidence,
    sample.diagnosis.futureRisk,
  ]);
  ...
  link.href = url;
  link.download = `predictive-maintenance-${stamp}.csv`;
  document.body.appendChild(link);
  link.click();
  link.remove();
  URL.revokeObjectURL(url);
  setStatus(`Saved ${state.samples.length} samples`);
}
...
elements.saveCsvBtn.addEventListener("click", saveCsv);
